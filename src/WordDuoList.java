...
public class WordDuoList {
    ...
    word that appears later in the array.  The allDuos list constains
    ...
    public WordDuoList(String[] words){
        ArrayList<WordDuo> output = new ArrayList<WordDuo>();
        for(int i = 0; i < words.length-1; i++)
        {
            for(int j = i+1; j < words.length; j++)
            {
                output.add(new WordDuo(words[i], words[j]));
            }
        }
        allDuos = output;
    }
    ...
    public void moveMatchesToTop(){
        ...
        {
            if(allDuos.get(i).first.equals(allDuos.get(i).second))
            {
                allDuos.add(0, allDuos.remove(i));
            }
        }
    }
